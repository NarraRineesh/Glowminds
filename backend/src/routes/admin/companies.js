...
//
// Every company doc carries enough config for `syncCompany` to pull jobs
// from its ATS. `jobsApi` and `careersUrl` are derived from the platform
// registry — we compute them server-side instead of trusting the client.
...
import { syncCompany } from "../../sync/syncCompany.js";
...
// POST /api/admin/companies/:slug/sync — runs syncCompany inline. Single
// company is fast enough (a few seconds) so we return the counters.
// Pass ?force=true to bypass the no-change guard.
router.post(
  "/companies/:slug/sync",
  requireAuth,
  requireAdmin,
  async (req, res, next) => {
    try {
      ...
      const result = await syncCompany(company, { force });
      // Re-read so the response shows updated counts + lastSyncAt.
      const refreshed = await db.collection("companies").doc(company.slug).get();
      res.json({
        result,
        company: serializeCompany(refreshed),
      });
    } catch (err) {
      next(err instanceof ApiError ? err : new ApiError("internal", err.message));
    }
