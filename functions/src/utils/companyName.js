 ...
 *      "purestorage" → "Pure Storage" (when known) / "Purestorage"
 */
export function formatCompanyDisplayName(raw) {
  ...
  // Workday / multi-tenant ATS: brand|wdN|site
  if (s.includes("|")) {
    s = s.split("|")[0].trim();
  }

  // Strip leading ats: prefix if present
  ...
  // Already looks like a proper display name (mixed case / spaces with capitals)
  ...
  // Humanize snake/kebab/camel-ish slugs
  s = s
